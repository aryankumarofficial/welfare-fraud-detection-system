    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="border border-border p-6">
          ...
          <CardDescription>
            Number of reviewed outcomes in the system.
          </CardDescription>
          <div className="mt-6 text-4xl font-semibold">
            {modelPerformance.reviewed_predictions}
          </div>
        </Card>
        <Card className="border border-border p-6">
          <CardTitle>Confirmed fraud</CardTitle>
          <CardDescription>
            Confirmed fraud decisions by reviewers.
          </CardDescription>
          <div className="mt-6 text-4xl font-semibold">
            {modelPerformance.confirmed_fraud_count}
          </div>
        </Card>
        <Card className="border border-border p-6">
          <CardTitle>False positives</CardTitle>
          <CardDescription>
            Predictions flagged incorrectly as fraud.
          </CardDescription>
          <div className="mt-6 text-4xl font-semibold">
            {modelPerformance.false_positives}
          </div>
        </Card>
      </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <Card className="border border-border">
          <CardHeader className="px-6 py-5">
            ...
            <CardDescription>
              Higher values indicate better review agreement.
            </CardDescription>
          </CardHeader>
          <CardContent className="px-6 pb-6 pt-0">
            <div className="flex items-center gap-4">
              ...
              <Badge variant={getVariant(modelPerformance.precision)}>
                {modelPerformance.precision >= 0.75
                  ? "Good"
                  : modelPerformance.precision >= 0.4
                    ? "Moderate"
                    : "Low"}
              </Badge>
            </div>
        ...
        <Card className="border border-border">
          <CardHeader className="px-6 py-5">
            ...
            <CardDescription>
              Share of reviewed predictions matching confirmed fraud decisions.
            </CardDescription>
          </CardHeader>
          <CardContent className="px-6 pb-6 pt-0">
            <div className="flex items-center gap-4">
              <p className="text-5xl font-semibold">
                {(modelPerformance.review_agreement_rate * 100).toFixed(1)}%
              </p>
              <Badge
                variant={getVariant(modelPerformance.review_agreement_rate)}
              >
                {modelPerformance.review_agreement_rate >= 0.75
                  ? "Stable"
                  : modelPerformance.review_agreement_rate >= 0.4
                    ? "Watch"
                    : "Review"}
              </Badge>
            </div>
      ...
      <Card className="mt-8 border border-border">
        <CardHeader className="px-6 py-5">
          ...
          <CardDescription>
            Counts by individual review decisions.
          </CardDescription>
        </CardHeader>
        <CardContent className="px-6 pb-6 pt-0 space-y-4">
          {Object.entries(modelPerformance.decision_counts).map(
            ([decision, count]) => (
              <div
                key={decision}
